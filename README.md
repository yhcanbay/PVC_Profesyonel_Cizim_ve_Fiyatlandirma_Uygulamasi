# 🪟 PVC Tasarım ve Dinamik Fiyatlandırma Sistemi
...
## 📝 Proje Hakkında
Bu proje, kullanıcıların PVC pencere ve kapı tasarımlarını görsel olarak yapabildiği, kanat yönleri, menteşe ve kol detaylarına kadar özelleştirebildiği ve bu tasarımlar üzerinden anlık, dinamik fiyatlandırma alabildiği yerel (local) çalışmak üzere tasarlanmış kapsamlı bir web uygulamasıdır. 
...
*   **🎨 Dinamik Görsel Çizim Motoru:** Kullanıcılar pencere veya kapı kanatlarını seçebilir; sağ/sol açılım yönlerine göre menteşe (`pencere_kolu.png`, `kapi_kolu.png` vb.) ve kolların otomatik ve doğru konumlarda render edilmesini sağlar.
...
*   **State Management:** Çizim durumu ve anlık sepet/fiyat takibi için kullanıldı.
...
*   **JWT (JSON Web Token):** Session-less (oturumuz) doğrulama altyapısı.

### Veritabanı
*   **Relational Database:** (Kullandığın DB'yi buraya yazabilirsin örn: PostgreSQL / MySQL). Nesne-ilişkisel eşleme (ORM) standartları kullanılarak tasarlandı.
...
*   **Görsel Hiyerarşi:** Çizim motorunda menteşe/kol gibi bileşenlerin kanatın *altında kalmaması* (z-index ve absolute positioning sorunları) başarılı bir render state mantığı kurularak çözündü.
*   **Fiyat Mimarisi Hizalaması:** Anlık etkileşim gerektiren karmaşık fiyat algoritması, sunucuya sürekli istek atmak yerine hesaplamayı Client (Frontend) tarafında anlık yapacak şekilde kurgulandı ve kaynak kullanımı optimize edildi.

---
**Geliştirici:** (Kendi Adın/Soyadın)
